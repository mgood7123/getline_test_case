...
// provide minimal capabiliry from builtins, enough to make the shell work
...
void execute_thread(struct command ** commands) {
	...
	for (t = 1; t < 2; t++) {
		// to enable background execution, without the ability to independantly exit, comment out from here
...
void parse_special_commands(int cc, char * command_list[]) {
	...
	for (commands.vector_line_idx = 0; commands.vector_line_idx < cc; commands.vector_line_idx++) {
        ...
        free(qt);
        for (int i = 0, i < strlen(cmd->vector_line[cmd->vector_line_idx])+1; i++) {
        	pi(*(cmd->vector_line[cmd->vector_line_idx]+i))
    		pc(*(cmd->vector_line[cmd->vector_line_idx]+i))
    	}
